...
# 1. Remove all old makeDraggable implementations
# We know they start with `function makeDraggable` and end just before `makeDraggable('companion-character-wrap'`
start = text.find('function makeDraggable')
end = text.rfind("makeDraggable('companion-character-wrap'") # rfind to get the LAST one just in case
...
# ensure window.isDraggingVivian doesn't duplicate
text = text.replace('window.isDraggingVivian = false;\n\n                                      window.isDraggingVivian = false;', 'window.isDraggingVivian = false;')
