...
public class PostController {
    ...
    public String board(@RequestParam(required = false) String region,
        ...
        model.addAttribute("posts", postService.boardList(region, PageRequest.of(page, 12)));
        return "board/list";
    }
    @GetMapping("/board/{id}")
    public String detail(@PathVariable Long id, @AuthenticationPrincipal CustomUserDetails principal, Model model) {
        ...
        model.addAttribute("comments", postCommentRepository.findByPostOrderByCreatedAtAsc(post));
