...
/**
 * 구글/네이버/라인이 주는 사용자 정보 형태가 각각 달라서 하나로 맞춰주는 변환기.
 * - 구글: {sub, email, name} 최상위
 * - 네이버: {response:{id, email, name}} 처럼 중첩
 * - 라인(OIDC userinfo): {sub, email, name} 최상위 - 구글과 형태는 같지만, email 스코프는
 *   LINE 측 별도 심사(비즈니스 계정 승인)가 필요해서 심사 전에는 email 이 아예 안 내려온다.
 *   그 경우 email=null 로 넘기고, 화면(signup-social)에서 직접 입력받는다.
 */
