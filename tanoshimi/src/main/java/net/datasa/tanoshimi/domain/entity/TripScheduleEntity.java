...
/**
 * 여행 계획표. 파티 하나당 1개이며 파티 생성 시점에 바로 만들어진다(party 로 연결).
 *
 * <p>파티에 연결된 tour 가 있으면 항공/체크인 등 package_default 블록이 추가되고,
 * 그 블록들은 이후 영구히 고정(수정 불가)된다.
 *
 * <p>draft: 자유 편집 -> submitted: 제출됨 -> confirmed: 최종 확정.
 */
@Entity
@Getter
...
public class TripScheduleEntity {
    ...
    /** 파티 생성 시점에 바로 계획표를 만든다. 편집권은 기본적으로 파티장이 가진다. */
